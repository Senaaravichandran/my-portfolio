...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    const drawCellFill = (key, drawShape) => {
      ...
      if (isHex) {
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            const key = `${col},${row}`;
            drawCellFill(key, () => drawHex(cx, cy, squareSize));
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            const key = `${col},${row}`;
            drawCellFill(key, () => drawTriangle(cx, cy, squareSize, flip));
        ...
        for (let col = -2; col < cols; col++) {
          for (let row = -2; row < rows; row++) {
            ...
            const cy = row * squareSize + squareSize / 2 + offsetY;
            const key = `${col},${row}`;
            drawCellFill(key, () => drawCircle(cx, cy, squareSize));
    ...
    const handleMouseMove = event => {
      ...
      if (isHex) {
        ...
        const offsetX = ((gridOffset.current.x % squareSize) + squareSize) % squareSize;
        const offsetY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize;
        setHovered(Math.round((mouseX - offsetX) / squareSize), Math.round((mouseY - offsetY) / squareSize));
      } else {
        const offsetX = ((gridOffset.current.x % squareSize) + squareSize) % squareSize;
        const offsetY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize;
        setHovered(Math.floor((mouseX - offsetX) / squareSize), Math.floor((mouseY - offsetY) / squareSize));
      }
    };
